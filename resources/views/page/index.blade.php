...
@if (session('success'))
    <div class="absolute max-w-4xl bottom-10 right-10 mx-auto mt-4 p-4 bg-green-100 border border-green-400 text-green-700 rounded">
        {{ session('success') }}
    </div>
@endif
...
@if (session('error'))
    <div class="absolute max-w-4xl bottom-10 right-10 mx-auto mt-4 p-4 bg-red-100 border border-red-400 text-red-700 rounded">
        {{ session('error') }}
    </div>
@endif
...
<div class="max-w-4xl mx-auto mt-10 mb-6">
    <a href="{{ route('pages.create') }}" 
       onclick="openPageCreateModal(event, '{{ route('pages.create') }}')" 
...
<div id="page-edit-modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
    <div id="page-edit-modal-content" class="bg-white rounded-lg shadow-lg p-6 max-w-lg w-full relative">
        <button onclick="closePageEditModal()"
...
<div id="page-create-modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
    <div id="page-create-modal-content" class="bg-white rounded-lg shadow-lg p-6 max-w-lg w-full relative">
        <button onclick="closePageCreateModal()"
            class="absolute top-2 right-2 text-gray-500 hover:text-black text-2xl">&times;</button>
...
<script>
    ...
    function openPageCreateModal(event, url) {
        event.preventDefault();
        fetch(url)
            .then(response => response.text())
            .then(html => {
                document.getElementById('page-create-modal-content').innerHTML =
                    '<button onclick="closePageCreateModal()" class="absolute top-2 right-4 text-gray-500 hover:text-black text-2xl">&times;</button>' +
                    html;
                document.getElementById('page-create-modal').classList.remove('hidden');
            });
    }

    function closePageCreateModal() {
        document.getElementById('page-create-modal').classList.add('hidden');
        document.getElementById('page-create-modal-content').innerHTML = '';
    }
